    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .chart-row { display: grid; grid-template-columns: 52px 1fr 140px 130px; align-items: center; gap: 16px; padding: 14px 20px; background: transparent; border-radius: 8px; cursor: pointer; transition: background 0.15s; text-decoration: none; color: inherit; }
      ...
      <div style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', color: '#fff', fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
        ...
        <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '52px 24px 80px' }}>
          ...
          <div style={{ marginBottom: '8px' }}>
            <Link href="/genre" style={{ fontSize: '13px', color: '#444', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px', marginBottom: '20px' }}>
              ← Alle Genres
            </Link>
          </div>
          ...
          <div style={{ marginBottom: '36px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '6px' }}>
              ...
              <h1 style={{ fontSize: '26px', fontWeight: '700', letterSpacing: '-0.5px', textTransform: 'capitalize' }}>
                {genre}
              </h1>
            </div>
            <p style={{ color: '#444', fontSize: '13px', paddingLeft: '18px' }}>
              {songs?.length ?? 0} KI-generierte Songs in diesem Genre
            </p>
          </div>

          {songs && songs.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '52px 1fr 140px 130px', gap: '16px', padding: '0 20px 10px', borderBottom: '1px solid #161616', marginBottom: '8px' }}>
          ...
          {songs && songs.length > 0 ? (
            ...
            <div style={{ marginTop: '24px', padding: '72px 32px', textAlign: 'center', border: '1px solid #161616', borderRadius: '12px', background: '#0d0d0d' }}>
              <p style={{ color: '#444', fontSize: '16px', marginBottom: '8px', fontWeight: '500' }}>
                Keine Songs in diesem Genre gefunden
              </p>
              <p style={{ color: '#2a2a2a', fontSize: '13px' }}>
                <Link href="/genre" style={{ color: '#ff6a00', textDecoration: 'none' }}>← Zurück zur Genre-Übersicht</Link>
              </p>
            </div>
          )}
        </main>
